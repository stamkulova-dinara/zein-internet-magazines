...
const Similar = ({ product }) => {
  ...
  return (
    <div className={style.card_slider} key={product.id}>
      ...
      >
        {likepage !== true ? (
          <FiHeart className={style.unlike} key={product.id} />
        ) : (
          <RiHeartFill className={style.like} key={product.id} />
        )}
      </div>
      <Link to={`/collections/${product.collectionId}/product/` + product.id}>
        ...
        <div className={style.card_info}>
          <h6>
            <span className={style.old_price}>{product.oldPrice} p</span>{" "}
            {product.price} p
          </h6>
